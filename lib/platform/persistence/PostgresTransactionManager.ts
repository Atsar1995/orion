...
export class PostgresTransactionManager implements TransactionManager {
  ...
  constructor(
    ...
    private readonly crmPersister?: CrmEntityPersister,
    private readonly procurementPersister?: ProcurementEntityPersister,
  ) {}
  ...
  async beginTransaction(): Promise<RepositoryResult<PersistenceTransaction>> {
    try {
      ...
      this.crmPersister?.beginTransaction();
      this.procurementPersister?.beginTransaction();
  ...
  async commit(transaction: PersistenceTransaction): Promise<RepositoryResult<void>> {
    ...
    try {
      ...
      await this.crmPersister?.flushPending();
      await this.procurementPersister?.flushPending();
      ...
      this.crmPersister?.endTransaction();
      this.procurementPersister?.endTransaction();
    }
  }
  ...
  async rollback(transaction: PersistenceTransaction): Promise<RepositoryResult<void>> {
    ...
    try {
      ...
      this.crmPersister?.discardPending();
      this.procurementPersister?.discardPending();
      return success(undefined);
    } catch (error) {
      return failure({
        code: PersistenceErrorCode.Storage,
