...
import { isSupportedProcurementFinanceEventType } from "@/lib/finance/integration/FinanceProcurementSupportedEvents";
...
export function registerFinanceEventSubscriptions(service: IntelligenceIntegrationService): void {
  ...
  service.subscribe(
    ...
    async (event, context) => {
      ...
      if (isSupportedProcurementFinanceEventType(event.payload.canonicalEventType)) {
        return;
      }
