...
export function BookingChannels({ channels = BOOKING_CHANNELS }: BookingChannelsProps) {
  return (
    <Card title="Booking Channels">
      <ul className="grid grid-cols-1 gap-3 sm:grid-cols-2">
        {channels.map((channel) => (
          <li key={channel.channel} className="rounded-orion-md border border-white/[0.05] bg-white/[0.02] px-4 py-3">
            ...
            <p className="mt-2 text-xs font-light leading-relaxed text-white/55">{channel.summary}</p>
          </li>
        ))}
      </ul>
    </Card>
