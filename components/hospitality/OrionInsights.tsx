...
export function OrionInsights({ insights = ORION_INSIGHTS }: OrionInsightsProps) {
  return (
    <Card title="ORION Insights" variant="premium">
      <ul className="grid grid-cols-1 gap-4 lg:grid-cols-3">
        {insights.map((item) => (
          <li key={item.category} className="rounded-orion-md border border-orion-gold/10 bg-orion-gold/[0.04] p-4">
            <p className="text-xs font-medium tracking-wide text-orion-gold/80 uppercase">{item.category}</p>
