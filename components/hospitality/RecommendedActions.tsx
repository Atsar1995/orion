...
type RecommendedActionsProps = {
  actions?: readonly { priority: number; title: string; description: string }[];
};

/** Prioritized hospitality action cards for owners and managers. */
export function RecommendedActions({ actions = RECOMMENDED_ACTIONS }: RecommendedActionsProps) {
  return (
    <Card title="Recommended Actions">
      <ul className="grid grid-cols-1 gap-3 lg:grid-cols-3">
        {actions.map((action) => (
          <li key={action.priority} className="rounded-orion-md border border-white/[0.05] bg-white/[0.02] p-4">
