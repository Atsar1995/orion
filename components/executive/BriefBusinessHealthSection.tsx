...
/** Business health card for the overview grid (Mission P-002). */
export function BriefBusinessHealthSection({ health }: BriefBusinessHealthSectionProps) {
  return <BusinessHealthCard health={health} />;
}

/** Major risks and opportunities below the health/alerts grid (Mission P-002). */
export function BriefBusinessHealthRiskOpportunitiesSection({
  health,
}: BriefBusinessHealthSectionProps) {
  if (health.majorRisks.length === 0 && health.majorOpportunities.length === 0) {
    return null;
  }
