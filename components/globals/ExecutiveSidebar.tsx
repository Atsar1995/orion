    ...
    >
      <NavIconGlyph
        ...
        className={cn(
          ...
          isActive ? "text-orion-gold" : "text-white/35 group-hover:text-white/60",
        )}
      />
...
/** Executive platform sidebar driven by the navigation registry. */
    ...
    <aside className="fixed inset-y-0 left-0 z-30 flex w-64 flex-col border-r border-white/[0.06] bg-orion-navy-deep/95 backdrop-blur-xl">
      <div className="flex items-center gap-3 border-b border-white/[0.06] px-5 py-5">
        ...
        <div className="min-w-0">
          <p className="text-base font-semibold tracking-tight text-white">ORION</p>
      ...
      <nav aria-label="Executive platform navigation" className="flex-1 space-y-6 overflow-y-auto px-3 py-5">
        {sections.map((section) => (
          <SidebarSection key={section.id} label={section.label} items={section.items} />
        ))}
      </nav>
    </aside>
  );
