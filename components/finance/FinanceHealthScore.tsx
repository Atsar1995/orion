...
import { FINANCE_HEALTH_SCORE } from "@/lib/finance-insights";

/** Financial health score with trend and driver breakdown. */
export function FinanceHealthScore() {
  return (
    <Card title="Financial Health Score" variant="premium">
        ...
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            ...
            <p className="mt-1 text-4xl font-semibold tracking-tight text-white">
              {FINANCE_HEALTH_SCORE.score}
              <span className="text-lg font-light text-white/40">/100</span>
            </p>
          </div>
          <div className="flex flex-col items-end gap-2">
            <StatusIndicator status={FINANCE_HEALTH_SCORE.status} />
            <StatCard label="Trend" value={FINANCE_HEALTH_SCORE.trend} />
          </div>
        </div>
        <p className={WORKSPACE_SUMMARY_CLASS}>{FINANCE_HEALTH_SCORE.summary}</p>
        <ul className="grid grid-cols-2 gap-2.5 sm:grid-cols-4">
          {FINANCE_HEALTH_SCORE.drivers.map((driver) => (
            <li
              key={driver.label}
